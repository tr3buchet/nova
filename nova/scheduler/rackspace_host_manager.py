...
class RackspaceHostState(host_manager.HostState):
    ...
    def update_from_instance(self, instance):
        super(RackspaceHostState, self).update_from_instance(instance)
    ...
    def __repr__(self):
        return "%s ram:%s disk:%s io_ops:%s instances:%s" % (self.host,
                self.free_ram_mb, self.free_disk_mb, self.num_io_ops,
                self.num_instances)
