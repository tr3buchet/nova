...
class BackupScheduleController(object):
    ...
    def index(self, req, server_id):
        ...
        return {"backupSchedule": dict(enabled=enabled, weekly=weekly,
                                       daily=daily)}
        ...
        if schedule['enabled']:
            ...
            if day_str != 'DISABLED':
                ...
                self.tempo_client.task_create(
                    'weekly_backup', server_id, recurrence)
            ...
            if hour_spec != 'DISABLED':
                ...
                self.tempo_client.task_create(
                    'daily_backup', server_id, recurrence)
    ...
    def _get_tempo_backups(self, server_id):
        ...
        for task in tasks:
            if task['instance_uuid'] == server_id:
                if task['task'] == 'daily_backup':
                    daily_backup = task
                elif task['task'] == 'weekly_backup':
                    weekly_backup = task
