...
class Quantum2ManagerGetInstanceNwInfo(test.TestCase):
    def test_get_instance_nw_info(self):
        ...
        network_ids, networks_stub = _get_all_networks_stub(network_count=1,
                                                    tenant_id=self.tenant_id)
        self.networks = {self.tenant_id: network_ids}
        self.stubs.Set(self.net_manager, '_get_quantum_tenant_nets',
                       _get_quantum_tenant_nets_stub(self.networks))
        vifs = [_vif_helper(self.tenant_id, network_ids[0]['network_id'])]

        stub(melange_connection.MelangeConnection,
                       'get_allocated_networks',
                       _get_allocated_networks_stub(vifs, bare_uuids=False))
        stub(self.net_manager, '_get_all_networks', networks_stub)

        get_nw_info = self.net_manager.get_instance_nw_info
        res = get_nw_info(self.context, instance_id=1,
    ...
    def test_get_instance_nw_info_correct_order(self):
        ...
        network_ids, networks_stub = _get_all_networks_stub(network_count=5,
                                                    tenant_id=tenant_id)
        networks = [{'id': n['network_id'], 'name': n['name']}
                     for n in network_ids]
        self.stubs.Set(net_manager, '_get_quantum_tenant_nets',
                       _get_quantum_tenant_nets_stub({tenant_id: networks}))
        self.stubs.Set(FLAGS, 'network_order',
                       ['net%d' % i for i in xrange(len(network_ids))])
        vifs = [_vif_helper(tenant_id, network_ids[-1]['network_id']),
                _vif_helper(tenant_id, network_ids[0]['network_id'])]

        stub(melange_connection.MelangeConnection,
                       'get_allocated_networks',
                       _get_allocated_networks_stub(vifs, bare_uuids=False))
        stub(net_manager, '_get_all_networks', networks_stub)

        res = net_manager.get_instance_nw_info(ctx, instance_id=1,
                                               project_id=tenant_id)
        network_ids.reverse()
        self.assertEqual(len(res), len(vifs))
        self.assertEqual(res[0]['network']['label'], networks[0]['name'])
        self.assertEqual(res[1]['network']['label'], networks[-1]['name'])
