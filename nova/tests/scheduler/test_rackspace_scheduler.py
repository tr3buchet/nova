...
class RackspaceSchedulerTestCase(test.TestCase):
    ...
    def test_schedule_one_at_a_time(self):
        ...
        self.stubs.Set(distributed_scheduler.DistributedScheduler,
                'schedule_run_instance', fake_schedule_run_instance)

        sched = rackspace_scheduler.RackspaceScheduler()

        thr1 = eventlet.spawn(sched.schedule_run_instance, None, dict(num=1))
        thr2 = eventlet.spawn(sched.schedule_run_instance, None, dict(num=2))
        res2 = thr2.wait()
        res1 = thr1.wait()
